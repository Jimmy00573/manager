...
async function dbGetVehicles() { try { return await sbGet('vehicles', 'order=number'); } catch(e) { _sbLoadFail('차량'); return []; } }
...
// ── 위치 마스터
async function dbGetLocations() { try { return await sbGet('storage_locations', 'order=sort_order,name'); } catch(e) { _sbLoadFail('보관 위치'); return []; } }
...
// ── 거래처
async function dbGetPartners() { try { return await sbGet('partners', 'order=sort_order.asc,name.asc'); } catch(e) { _sbLoadFail('거래처'); return []; } }
...
async function dbGetManualTransactions() { try { return await sbGet('manual_transactions', 'is_void=eq.false&order=date.desc,created_at.desc'); } catch(e) { _sbLoadFail('수동 거래내역'); return []; } }
...
async function dbGetContainerTypes() { try { return await sbGet('container_types', 'order=sort_order.asc,id.asc'); } catch(e) { _sbLoadFail('콘테이너 종류'); return []; } }
...
// ── 파치 사용처
async function dbGetPachiUsages() { try { return await sbGet('pachi_usages', 'order=sort_order'); } catch(e) { _sbLoadFail('파치 사용처'); return []; } }
...
// ── 당도(브릭스) 등급 마스터
async function dbGetBrixGrades() { try { return await sbGet('brix_grades', 'order=sort_order'); } catch(e) { _sbLoadFail('당도 등급'); return []; } }
...
async function dbGetPachiSizes() { try { return await sbGet('pachi_sizes', 'order=sort_order'); } catch(e) { _sbLoadFail('파치 크기'); return []; } }
...
async function dbGetPachiConditions() { try { return await sbGet('pachi_conditions', 'order=sort_order'); } catch(e) { _sbLoadFail('파치 상태'); return []; } }
...
// ── 품질 기준
async function dbGetQualityCriteria() { try { return await sbGet('quality_criteria', 'order=product_name'); } catch(e) { _sbLoadFail('품질 기준'); return []; } }
...
async function loadBrixMaxSize() {
  try {
    ...
    if (rows && rows.length > 0) return rows[0].value || {};
  } catch(e) { _sbLoadFail('당도 최대 사이즈 설정'); }
  return {};
}
...
async function dbGetJuiceBatches() {
  try { return await sbGet('juice_batches', 'is_void=eq.false&order=inbound_date.desc,created_at.desc'); } catch(e) { _sbLoadFail('주스·청 배치'); return []; }
}
...
async function dbGetJuiceMasters() {
  try { return await sbGet('juice_product_master', 'is_active=eq.true&order=product_name'); } catch(e) { _sbLoadFail('주스 제품'); return []; }
}
...
// ── 카테고리 시스템
async function dbGetCategories() { try { return await sbGet('categories', 'order=id'); } catch(e) { _sbLoadFail('품목 분류'); return []; } }
...
async function dbGetSizeGrades() { try { return await sbGet('size_grades', 'order=sort_order'); } catch(e) { _sbLoadFail('사이즈 등급'); return []; } }
...
async function dbGetItems() { try { return await sbGet('items', 'order=name'); } catch(e) { _sbLoadFail('품목'); return []; } }
...
async function dbGetItemSizeRules() { try { return await sbGet('item_size_rules', 'order=item_id,min_su'); } catch(e) { _sbLoadFail('품목 사이즈 규칙'); return []; } }
...
async function dbGetSortingResults(inboundIds) {
  ...
  try { return await sbGet('sorting_results', q); } catch(e) { _sbLoadFail('선과 결과'); return []; }
}
