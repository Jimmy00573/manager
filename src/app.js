...
//   그 종류는 2026-08-18 DB에서 제거됐지만, 접두가 겹치는 이름이 또 생길 수 있으므로 규칙은 유지한다.
const OT_ALIAS = { '노랑': '황제', '초록': '시트리앙', '헌콘': '헌콘테이너' };
function ctNorm(v) { return OT_ALIAS[v] || v || ''; }

// ★콘테이너 종류 아이콘 — 전 화면 단일 소스. 종류가 늘면 여기만 고친다.
//   실물 색이 각양각색이라 색으로는 못 맞추므로 소유별 계열로 구분한다.
//   맵에 없는 종류는 _ctIcon()이 📦로 폴백 — 새 종류가 생겨도 아이콘이 비지 않는다.
const _CT_ICON = {
  황제: '🟡', 시트리앙: '🟢', 헌콘테이너: '⬜',   // 우리것 — 기존 색 유지(색으로 구분하던 습관 보존)
  농가: '🟠', 사각: '🔲',                          // 농가것
  콘테이너: '🔵', 파렛트: '🟣',                    // 농협·거래처것
...
let brixMaxSize = {};   // 브릭스 분리 최대 사이즈 설정(예 {'감귤류':'M2'}) — 초과 사이즈는 저장 시 일반 합산
...
function showConfirmDanger({ title, subtitle = '복구할 수 없는 작업입니다', items = [], resultNote = '', confirmText = '삭제', cancelText = '취소', needWorker = false }) {
  return new Promise(resolve => {
    ...
    // 바깥(오버레이) 클릭으로는 안 닫힘 — 실수로 닫혀 실사·입력 손실되는 것 방지(ESC/취소로만 닫힘)
...
// 소유 판별(그룹·KPI용). nhf명+종류의 대표 owner_type. gNhfSt는 nhf명 기준(불변).
function nhfOwner(nhf, type) {
  const rec = nhfIns.find(o => o.nhf === nhf && o.type === type) || nhfOuts.find(o => o.nhf === nhf && o.type === type);
  return (rec && rec.owner_type) || '농협';
}
...
// ★종류 제한 없음 — 농가것 종류는 사각·농가 등 전부 콘테이너라 농협의 '콘테이너 포함' 조건을 가져오지 않음.
...
function renderNhf() {
  ...
  const ownerRank = k => { const [n, t] = k.split('||'); return nhfOwner(n, t) === '농협' ? 0 : 1; };   // 소유 그룹 정렬(농협 먼저 → 거래처)
  pend.sort((a, b) => ownerRank(a) - ownerRank(b));
  done.sort((a, b) => ownerRank(a) - ownerRank(b));
  const ownerBadge = owner => `<span class="badge ${owner === '농협' ? 'b-teal' : 'b-info'}">${esc(owner)}</span>`;
...
// ── 현황판
// 농가보유 집계 — target_type '농가'만(null 포함=구데이터). 농협/거래처行 배차는 getTargetContainerHold로 별도.
...
function getTargetContainerHold(name, targetType) {
  ...
  const rec = picks.filter(p => p.farm === name && (p.type === '원물수거' || p.type === '빈콘회수') && p.target_type === targetType).reduce((s, p) => s + p.qty, 0);
...
// ── [화면: 수확·수송 > 콘테이너 통합 이력(chist 탭)] 4소스 통합 조회 전용. 계산·저장 없음.
...
// ── [화면: 수확·수송 > 현황판] KPI + 🟡회수 필요(농가行·농협行·거래처行) + 🟢반납 필요. 농가 표는 renderFarmTbl.
function renderDash() {
  const isAdm = sessionStorage.getItem('citrus_role') === 'admin';
  ...
  document.getElementById('afc').textContent = (fhi.length + nhfHoldList.length + ptHoldList.length) + '곳 · ' + (tfTotal + nhfHoldTotal + ptHoldTotal) + '개';
  ...
  document.getElementById('arb').innerHTML = allReturns.length ? allReturns.map(i => {
    const badge = i.kind === 'own'
      ? `<span class="badge" style="background:#F3E5F5;color:#6A1B9A">농가것</span>`
      : i.kind === 'partner'
        ? `<span class="badge b-info">거래처</span>`
...
// ── [화면: 수확·수송 > 수확 캘린더] 월 그리드 + 금일 strip + 등록 폼. 내부에서 renderHarvestActive/renderCalUpcoming 호출.
function renderCal() {
...
// ── [화면: 수확·수송 > 수확 캘린더 > 예정 목록] renderCal에서 호출. 진행중은 renderHarvestActive.
function renderCalUpcoming() {
  ...
  el.innerHTML = page.map(e => {
    ...
    return `<div style="display:flex;align-items:flex-start;gap:10px;padding:9px 14px;border-bottom:0.5px solid #f0f0f0;border-left:3px solid ${lColor}">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;font-size:14px;color:#222">${esc(e.farm)}${e.round?` <span style="font-weight:600;font-size:11px;color:#1565C0">${e.round}차</span>`:''} ${e.item ? `<span style="font-weight:500;font-size:11px;padding:2px 7px;border-radius:4px;margin-left:4px;${itemColor(e.item)}">${esc(e.item)}</span>` : ''}</div>
...
async function unfinishAllHarvest(id) {
  ...
  if (!h) return;
  if (sessionStorage.getItem('citrus_role') !== 'admin') return alert('관리자만 가능합니다.');
  if (!(await showConfirmEdit('수확 종료 해제', `${h.farm} ${h.round||1}차 수확의 전체 종료를 해제할까요?\n다시 차수를 추가할 수 있습니다.`))) return;
  harvests = harvests.map(x => x.id === id ? { ...x, is_final: false } : x);   // 로컬 먼저 반영
  renderCal();
  try {
    await dbUpdateHarvest(id, { is_final: false });
  } catch (e) { alert('오류: ' + e.message); }
}
...
// ═══════════════════ 공용 유틸 · 마스터 설정 (CSV·엑셀 · 위치 · 품질 · 크기 기준) ═══════════════════
// ── CSV 공용 헬퍼
...
// 브릭스 분리 최대 사이즈 변경 — settings key='brix_max_size' 저장, 실패 시 롤백
async function chgBrixMaxSize(v) {
  if (sessionStorage.getItem('citrus_role') !== 'admin') return alert('관리자만 가능합니다.');
  const prev = { ...brixMaxSize };
  brixMaxSize = { ...brixMaxSize, '감귤류': v };
  try {
    await saveBrixMaxSize(brixMaxSize);
    showToast(`브릭스 분리 최대 사이즈: ${v}`);
  } catch (e) {
    brixMaxSize = prev; renderBrixGradeCfg();
...
// 오늘 '선과품으로 입고'된 재고인가 — 물건이 아직 들어오는 중일 수 있어 매트릭스에서 색으로 구분하고,
...
function renderInventoryStatus() {
  ...
  const matrixHtml = Object.entries(byProduct)
    .sort(([a], [b]) => a.localeCompare(b, 'ko'))
    .map(([product, productRecs]) => _renderInvMatrix(product, productRecs, _invAuditMode))
    .join('');

  if (_invAuditMode) {
    // 실사 대상 = 현재 표시 재고(수량>0). 실사 초기화·미확인삭제가 이 목록을 참조.
    _invAuditVisible = recs.filter(r => (Number(r.quantity) || 0) > 0);
    matrixEl.innerHTML = _renderInvAuditBar() + matrixHtml;
  } else {
...
function _renderInvMatrix(product, recs, auditMode) {
  ...
  recs.forEach(r => {
    ...
    if (!batchMap[key]) batchMap[key] = { farm, groupId, sortingDate, inboundDate, sizes: {}, recsBySize: {} };
    batchMap[key].sizes[sz] = (batchMap[key].sizes[sz] || 0) + (Number(r.quantity) || 0);
    (batchMap[key].recsBySize[sz] = batchMap[key].recsBySize[sz] || []).push(r);
  ...
  batches.forEach((batch, i) => {
    ...
    Object.keys(batch.recsBySize).forEach(sz => {
      recIds[sz] = batch.recsBySize[sz]
        .filter(r => (Number(r.quantity) || 0) > 0 && !r.is_void)
        .map(r => r.id);
    });
    ...
    displaySizes.forEach(sz => {
      ...
      // 이 셀에 '오늘 선과품 입고분'이 섞여 있으면 수량 색을 달리한다(값 0인 '-'·'＋'는 대상 아님).
      // ★색은 모드별로 '그 모드가 안 쓰는 색'을 골라 눈에 띄게 — 조회는 빨강, 실사는 검정(아래 참조).
      ...
      if (!audit) {
        const inner = val === 0
          ...
          : `<strong style="color:${cellToday ? '#DC2626' : '#111827'}">${fmtCT(val)}</strong>`;   // 조회 모드는 검정뿐이라 빨강이 튄다
      ...
      // ★실사 모드는 미확인(빨강)·확인(보라)만 쓰므로 여기선 검정이 오히려 튄다(확인해도 색 안 바뀜).
      //   단 검정은 '색 없음'처럼 읽힐 수 있어 당일 입고분만 한 단계 더 굵게(800) — 배경·테두리·✓는 기존 그대로.
      const numColor  = cellToday ? '#111827' : (allChk ? '#7C3AED' : '#B91C1C');
      const numWeight = cellToday ? ';font-weight:800' : '';
...
// 실사 진행률 바 (매트릭스 상단). _invAuditVisible 기준(수량>0 레코드).
function _renderInvAuditBar() {
  const items = _invAuditVisible || [];
...
async function invSgExcelRegister() {
  ...
  const kgPer = (productWeights && productWeights[product] != null) ? Number(productWeights[product]) : 17;
  const recs = [];
  const recMap = {};   // (등급×사이즈) 합산 — 기준 초과 브릭스→일반 전환 시 같은 사이즈 일반과 하나로(중복 행 방지)
  _SG_POS.forEach((pos, idx) => {
...
function renderOutboundHistory() {
  ...
  const txManual = manualTransactions.filter(r => !r.is_void).map(r => ({
    ...
    qty: Number(r.quantity)||0, unit: r.unit||'CT', partner: r.partner_name||'',
    amount: Number(r.amount)||0, source_type: 'manual_tx', expiry_date: null, farm: r.farm_name||'', manual: true, _raw: r
  }));
  ...
  function txRowHtml(t) {
    ...
    if (isAdmin && t.manual) {
      actionCell += `<button onclick="openManualTxModal('${r.id}')" style="background:none;border:1px solid #93C5FD;color:#2563EB;font-size:11px;padding:3px 8px;border-radius:5px;cursor:pointer;margin-right:4px">수정</button>`
...
function openManualTxModal(editId = null) {
  if (sessionStorage.getItem('citrus_role') !== 'admin') return;
  const ed = editId ? manualTransactions.find(r => String(r.id) === String(editId)) : null;
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width:460px;width:100%;background:#fff;border-radius:12px;padding:0">
      <div style="padding:16px 20px 12px;border-bottom:1px solid #F0F0F0;display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:16px;font-weight:700">✍️ 수동 거래 ${ed ? '수정' : '등록'}</div>
  ...
  if (ed) {
    ...
    _mtxSetDir(_mtxDir);   // 토글·거래처 라벨(출고처/입고처) 갱신
    if (g('mtx-date'))    g('mtx-date').value    = ed.date || td();
    if (g('mtx-partner')) g('mtx-partner').value = ed.partner_name || '';
    if (g('mtx-farm'))    g('mtx-farm').value    = ed.farm_name || '';
    if (g('mtx-product')) g('mtx-product').value = ed.product || '';
    _mtxUpdateSizes();     // 선택 품목 기준 사이즈 옵션 채우기
    if (g('mtx-size'))    g('mtx-size').value    = ed.size_code || '';
...
// 수동 거래 삭제 (soft delete, 재고 무관)
async function deleteManualTx(id) {
  if (sessionStorage.getItem('citrus_role') !== 'admin') return;
  const r = manualTransactions.find(x => String(x.id) === String(id));
  if (!r) return;
...
function _isBrixAllowedSize(sz, product) {
  if ((PRODUCT_TYPE_MAP[product] || '만감류') !== '감귤류') return true;   // 만감류는 전체 허용
  const iMax = SIZES_감귤류.indexOf(_brixMaxSizeOf());
  const i = SIZES_감귤류.indexOf(sz);
...
// ── [화면: 재고관리 > 입고내역 > 농가별 보기] 뷰 모드. 기본 목록은 renderInboundList.
...
// ── [화면: 재고관리 > 입고내역 > 카테고리별 보기] 뷰 모드. 기본 목록은 renderInboundList.
...
// ── [화면: 재고관리 > 입고내역 > 선과완료 보기] 뷰 모드. 기본 목록은 renderInboundList.
...
function startChipEdit(key, event) {
  ...
  inp.select();
  let committed = false;
  const commit = (confirmCell) => {
    if (committed) return;
    committed = true;
    commitChipEdit(key, inp.value, confirmCell);
  };
  const cancel = () => {
    if (committed) return;
    committed = true;
  ...
  inp.addEventListener('keydown', e => {
    if (e.key === 'Enter') { e.preventDefault(); commit(true); }   // 엔터=저장+확인(실사)
    else if (e.key === 'Escape') { e.preventDefault(); cancel(); }
  });
  inp.addEventListener('blur', () => commit(false));   // 블러=저장만(의도치 않은 확인 방지)
}
async function commitChipEdit(key, rawVal, confirmCell = false) {
  ...
  if (newVal === Number(rec.quantity)) {
    ...
    // 실사 모드 + 엔터(명시적 확정): 값이 안 바뀌어도 그 셀을 확인 처리(기존 audit_checked_at set 재사용)
    if (confirmCell && _invAuditMode && !rec.audit_checked_at) {
      try {
        const nowTs = new Date().toISOString();
        await sbUpdate('inventory_records', rec.id, { audit_checked_at: nowTs });
...
async function openSortingDetailModal(inboundId) {
  ...
  details.forEach(d => {
    ...
    if (d.category === '정상' && d.size_code) {
      ...
      const g = gradeOf(d);   // 등급별 맵(일반/11.5br/12br/잔존 고당…)
      rd.byGrade[g] = rd.byGrade[g] || {};
      rd.byGrade[g][d.size_code] = (rd.byGrade[g][d.size_code] || 0) + Number(d.ct);
    }
    else if (d.category === '파치')   rd.waste    += Number(d.ct);
    else if (d.category === '고산도') rd.highacid += Number(d.ct);
    else if (d.category === '저당도') rd.lowbrix  += Number(d.ct);
  ...
  details.filter(d => d.category === '정상' && d.size_code).forEach(d => {
    cumSizeMap[d.size_code] = (cumSizeMap[d.size_code] || 0) + Number(d.ct);
    const g = gradeOf(d);
    cumByGrade[g] = cumByGrade[g] || {};
    cumByGrade[g][d.size_code] = (cumByGrade[g][d.size_code] || 0) + Number(d.ct);
  });
  ...
  const orderGrades = present => {
    const activeBrix = brixGrades
      ...
      .map(g => g.label);
    const others = present.filter(g => g !== '일반' && !activeBrix.includes(g)).sort((a, b) => a.localeCompare(b, 'ko'));
    return ['일반', ...activeBrix, ...others].filter(g => present.includes(g));
  };
  // 등급별 사이즈 블록(0건 등급 제외). 일반=회색 라벨+문맥색 숫자, 브릭스/잔존=파랑.
  const gradeBlocks = (byGrade, normalColor) => orderGrades(Object.keys(byGrade)).map(g => {
    const sm = byGrade[g] || {};
    const gt = allSizes.reduce((s, sz) => s + (sm[sz] || 0), 0);
    if (gt <= 0) return '';
    const isNorm = g === '일반';
    const labelColor = isNorm ? '#6B7280' : '#1565C0';
    const numColor   = isNorm ? normalColor : '#1565C0';
    return `<div style="font-size:10px;font-weight:600;color:${labelColor};margin-top:6px">${esc(g)} <span style="color:#9CA3AF;font-weight:500">${fmtN(gt)} CT</span></div>${_sizeGroupCols(allGroups, sm, numColor)}`;
  }).join('');
